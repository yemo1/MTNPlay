...
namespace RSSFeed
{
    public partial class Hadith : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            //{
            //    string sql = "SELECT contents FROM R_General where category=@category and  ID=1";
            //    Feeds feed = new Feeds();
            //    feed.setFeed(sql, Response);
            //}
            //else
            //{
                //string sql = "SELECT * FROM R_General where category=@category and ID = 1";
                string sql = @"if not exists 
                    ...
                    order by g.ID asc";
                Feeds feed = new Feeds();
                feed.setFeeds(sql.Replace("{cat}", "Hadith"), Response);
            //}
        }
    }
}