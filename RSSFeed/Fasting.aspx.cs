...
namespace RSSFeed
{
    public partial class Fasting : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            //else
            //{
                //string sql = "SELECT * FROM R_General where category=@category and id= 127";
            string sql = @"if not exists 
                    ...
                    order by g.ID asc";
            Feeds feed = new Feeds();
            feed.setFeeds(sql.Replace("{cat}", "Fasting"), Response);
            //}
        }
    }
}